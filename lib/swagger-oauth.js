  ...
  popupDialog = $(
    [
      ...
      '<div class="api-popup-content">',
        ...
        '</ul>',
        '<p class="error-msg"></p>',
        '<label>User: </label><input type="text" id="username"><label>Password: </label><input type="password" id="password">',
  ...
  popupDialog.find('button.api-popup-authbtn').click(function() {
    ...
    for (var key in authSchemes) {
      if (authSchemes.hasOwnProperty(key)) {
        ...
        } else if(authSchemes[key].type === 'oauth2' && flow && (flow === 'password')) {
          oauth2KeyName = key;
          var dets = authSchemes[key];
          window.swaggerUi.tokenName = dets.tokenName || 'access_token';
          window.swaggerUi.tokenUrl = dets.tokenUrl;
          processOAuthPassword($('#username').val(), $('#password').val());
          return;
        }
...
function processOAuthPassword(username, password) {
   var params = {
    'client_id': clientId,
    'grant_type': 'password',
    'username': username,
